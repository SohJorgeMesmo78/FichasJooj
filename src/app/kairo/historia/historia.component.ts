...
export class HistoriaComponent implements OnInit, OnDestroy {
  ...
  capitulos: Capitulo[] = [
    ...
    },
    {
      ...
      paragrafos: [
        ...
        'Nailo olhou para a porta, depois para as correntes pesadas, e caminhou até o gigante azul. Ignorando as regras complexas da magia arcana, ele esticou a mãozinha e levantou o dedo mindinho em direção ao peito do Djinni.',
    ...
    },
    {
      ...
      estacao: 'verao',
      paragrafos: [
        'Assim que a porta negra bateu às suas costas, Kairo se viu no meio de uma planície vasta, espremida entre as encostas de uma grande montanha e a floresta densa e mística de <em>Mythos</em>. Foi só quando o silêncio da noite o abraçou que o garoto arregalou os olhos: em meio a toda a confusão e promessas, ele havia esquecido de perguntar o nome da entidade milenar que acabou de salvar sua vida. A partir daquele dia, o ser todo-poderoso seria chamado apenas de "Gênio", "Amigão" ou "Azulão".',
    ...
    },
    {
      ...
      paragrafos: [
        ...
        'Aquele era o seu <em>Livro das Sombras</em>. Ao folhear as páginas rúnicas, Kairo encontrou feitiços novos, mas um em especial chamou sua atenção: <em>Encontrar Familiar</em>. Naquela mesma noite, em seu quarto alugado em Ozenes, Kairo conjurou a magia. Das sombras e da luz feérica, surgiu <strong>Shift</strong>. O pequeno companheiro animal — que mudava de forma dependendo do dia, ora um lagarto sorrateiro, ora um falcão ágil — tornou-se seu companheiro.',
  ...
  ngOnDestroy() {
    // Restaurar a classe original do body ao sair da página de história
    if (typeof document !== 'undefined') {
      document.body.className = this.originalBodyClass;
    }
  }

  get currentCapitulo(): Capitulo {
    return this.capitulos[this.currentStep];
  }

  goToStep(index: number) {
    if (index >= 0 && index < this.capitulos.length) {
  ...
  nextStep() {
    if (this.currentStep < this.capitulos.length - 1) {
      this.currentStep++;
      this.applyTheme();
      this.scrollToTop();
    }
  ...
  applyTheme() {
    if (typeof document !== 'undefined') {
      const estacao = this.capitulos[this.currentStep].estacao;
